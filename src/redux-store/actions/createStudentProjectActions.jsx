...
import * as selectProjectVisibilityActions from "./selectProjectVisibilityActions";
...
export const loadData = async (params) => {
    return (dispatch, getState) => {
        ...
        if (!currentStudent
            ||
            (currentStudent && currentStudent.type === DB_CONST.TYPE_TEACHER)
        ) {
        ...
        if (params.edit || projectIDToBeLoadedAfterSavingFirstTime) {
            ...
            realtimeDBUtils
                ...
                .then(project => {
                    ...
                    if ((currentStudent.type === DB_CONST.TYPE_PROF && currentStudent.anid === project.anid)
                        && (courseProperties && courseProperties.anid === project.anid)
                    ) {
                        dispatch({
        ...
        // in create mode
            ...
            if (currentStudent.type === DB_CONST.TYPE_PROF
                && (courseProperties && courseProperties.anid === currentStudent.anid)
            ) {
                dispatch({
                    type: selectProjectVisibilityActions.SELECT_PROJECT_VISIBILITY_SET_PROJECT,
                    project: null
