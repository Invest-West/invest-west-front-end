import {StudentProfileAction, setCopiedStudent} from "../../StudentProfileActions";
...
export interface EditImageDialogAction extends StudentProfileAction {
    ...
    return async (dispatch: Dispatch, getState: () => AppState) => {
        ...
        try {
            ...
            const imageUrl = await new FileRepository().uploadSingleFile({
                ...
                fileName: storageID,
                storageLocation: `${STUDENTS_CHILD}/${copiedStudent.id}/${STUDENT_PROFILE_PICTURES_CHILD}`
            });

            // update student
            const response = await new StudentRepository().updateStudent({
                updatedStudent: JSON.parse(JSON.stringify(copiedStudent)),
                newProfilePicture: {
                    storageID,
                    url: imageUrl
                }
            ...
            dispatch(updateStudentChanges(updatedStudent));
            return dispatch(setCopiedStudent(updatedStudent));
        } catch (error) {
